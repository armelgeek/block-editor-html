...
export const handlePlay = async (
  ...
  if (!list?.length)
    list = (await getListDetail(id, leaderBoardState.filters, true)).list;
  if (list?.length) {
    await setTempList(listId, [...list]);
    void playList(LIST_IDS.TEMP, index);
    isPlayingList = true;
  }
  const fullList = await getListDetailAll(id, leaderBoardState.filters);
...
export const handleCollect = async (
  ...
  const list = await getListDetailAll(id, leaderBoardState.filters);
  await createList({
    name,
    id: listId,
    list,
