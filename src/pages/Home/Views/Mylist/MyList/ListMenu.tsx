  ...
  const handleSetMenu = (listInfo: LX.List.MyListInfo) => {
    ...
    switch (listInfo.id) {
      ...
      default:
        ...
        rename = true
        remove = true
        sync = !!(userList.source && musicSdk["kw"]?.songList)
        break
    }

    setMenus([
      { action: 'rename', disabled: !rename, label: 'list_rename' },
      { action: 'sync', disabled: !sync, label: 'list_sync' },
      { action: 'import', label: 'list_import' },
      { action: 'export', label: 'list_export' },
      // { action: 'local_file', label: t('list_select_local_file') },
      // { action: 'changePosition', label: t('change_position') },
      { action: 'remove', disabled: !remove, label: 'list_remove' },
    ])
  }
