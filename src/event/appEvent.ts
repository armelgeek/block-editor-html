...
export class AppEvent extends Event {
  ...
  mylistUpdated(
    lists: Array<
      LX.List.MyDefaultListInfo | LX.List.MyLoveListInfo | LX.List.UserListInfo
    >
  ) {
  ...
  songFilterChanged() {
    this.emit("songFilterChanged", leaderBoardState.filters);
  }
  userAccountUpdateChange(){
    this.emit("userAccountUpdateChange",null);
  }
  selectTagChange(tag:any){
    this.emit("selectTagChange",tag);
  }
}
