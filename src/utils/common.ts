...
export const formatPlayTime3 = (time: number) => {
  let m = Math.trunc(time / 60);
  let s = Math.trunc(time % 60);
  let ms = time % 1000;
  return numFix(m) + ":" + numFix(s) + "." + pad(ms, 3);
};
...
const fileNameRxp = /[\\/:*?#"<>|]/g;
export const filterFileName = (name: string): string =>
  name.replace(fileNameRxp, "");
