...
export const getUserTheme = async() => (await getData<any>(themeKey) ?? [])
export const saveUserTheme = async(themes: any) => {
...
export const saveSearchSetting = async(setting: Partial<typeof DEFAULT_SETTING['search']>) => {
  ...
  if(setting.flx && searchSetting.flx != setting.flx) requiredSave = false
...
export const getSongListSetting = async() => {
  console.log('getSongListSetting');
