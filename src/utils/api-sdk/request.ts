...
const handleRequestData = async (
  ...
  if (method.toLocaleLowerCase() === "post" && !headers["Content-Type"]) {
    if (options.form) {
      ...
      const formBody = [];
      for (let [key, value] of Object.entries(options.form) as any) {
...
const fetchData = (url: string, { timeout = 15000, ...options }: any) => {
  //console.log("---start---", url);

  return {
    request: handleRequestData(url, options).then((options: any) => {
      return fetch(url, {
          ...options,
          mode: 'no-cors',
          credentials: 'include'
        ...
        .then((resp: any) =>
          (options.binary ? resp.blob() : resp.text()).then((text: string) => {
            return {
