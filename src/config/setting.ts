...
export const updateSetting = (
  setting?: Partial<LX.AppSetting> | null,
...
export const initSetting = async () => {
  let setting: Partial<LX.AppSetting> | null = await getData(
    storageDataPrefix.setting
  );

  // try migrate setting before v1
  if (!setting) {
    const config = await getData<{ setting?: any }>(
      storageDataPrefixOld.setting
    );
    if (config != null) {
