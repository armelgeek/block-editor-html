import { arrPush, arrUnshift, formatPlayTime2, formatPlayTime3 } from '../../utils'
import { convertTimeToTag } from '../../utils/lyric';
...
export default {
  ...
  setPlayMusicInfo(listId: string | null, musicInfo: LX.Download.ListItem | LX.Music.MusicInfo | null, isTempPlay: boolean = false, isStudioPlay: boolean = false) {
    state.playMusicInfo = { listId, musicInfo, isTempPlay, isStudioPlay }

    global.state_event.playMusicInfoChanged(state.playMusicInfo)
  },
  setMusicInfo(_musicInfo: Partial<LX.Player.MusicInfo>) {
  // console.log('setMusici lalala',musicInfoKeys)
  ...
  setNowPlayTime(time: number) {
    ...
    state.progress.nowPlayTimeStrWithMs = formatPlayTime3(time)
  ...
  setProgress(currentTime: number, totalTime: number) {
    ...
    state.progress.nowPlayTimeStrWithMs = formatPlayTime3(currentTime)
  ...
  setModeCreator(modeCreator: boolean) {
    state.modeCreator = modeCreator
    global.state_event.playStateModeCreatorChanged(modeCreator)
  },
  addToStudioPlayList(list: any[]) {
    state.studioPlayList = list;
    global.state_event.playStudioPlayListChanged({ ...state.studioPlayList })
  },
  clearStudioPlayList() {
    state.studioPlayList = []
