...
const setLists = (
  ...
  listInfo.page = page;
  listInfo.list = deduplicationList(
    page > 1 ? [...listInfo.list, ...list] : list
  );
  state.source = "all";
...
const setList = (
  ...
  const list = datas.list.map(
    (s) => toNewMusicInfo(s) as LX.Music.MusicInfoOnline
  );
  listInfo.list = deduplicationList(
    page == 1 ? list : [...listInfo.list, ...list]
  );
  if (page == 1 || (datas.total && datas.list.length))
