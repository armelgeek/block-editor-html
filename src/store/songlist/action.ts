...
export default {
  ...
  setList(result: ListInfo, tagId: string, sortId: string, page: number) {
    state.listInfo.list =
      page == 1 ? [...result.list] : [...state.listInfo.list, ...result.list];
    if (page == 1 || (result.total && result.list.length))
  ...
  setListDetail(result: ListDetailInfo, id: string, page: number) {
    state.listDetailInfo.list =
      page == 1
        ? [...result.list]
        : [...state.listDetailInfo.list, ...result.list];
    state.listDetailInfo.id = id;
    ...
    state.listDetailInfo.maxPage = Math.ceil(
      state.listDetailInfo.total / result.limit
    );
  ...
  setSelectListInfo(info: ListInfoItem) {
    ...
    state.selectListInfo.time = info.time;
    state.selectListInfo.total = info.total;
  },
};
