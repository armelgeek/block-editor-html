...
export const getTags = async <T extends LX.OnlineSource>(source: T) => {
  if (songlistState.tags[source])
    return songlistState.tags[source] as TagInfo<T>;
  const info = await (musicSdk["kw"]?.songList.getTags() as Promise<
    TagInfo<T>
  >);
 ...
 * @param sortId
 */
export const setListInfo: typeof songlistActions.setListInfo = (
  source,
  tagId,
  sortId
) => {
 ...
 * @returns
 */
export const setList: typeof songlistActions.setList = (
  result,
  tagId,
  sortId,
  page
...
export const getList = async (
  ...
  let pageKey = `slist__${source}__${tabId}__${page}`;
  let listCache = cache.get(pageKey) as ListInfo;
  if (listCache && !_.isUndefined(listCache)) {
    if (isRefresh) cache.delete(pageKey);
    else return listCache;
  }
  //console.log('listiddd',filterString(boardState.listDetailInfo.id));
  return musicSdk["kw"]?.songList
    .getList(page, boardState.filters, tabId)
    .then((result: ListInfo) => {
      if (!_.isUndefined(result)) {
        //   console.log('result get list',result,pageKey)
...
const getListDetailLimit = async (
  ...
  const listKey = `sdetail__${"kw"}__${id}`;
  const prevPageKey = `sdetail__${"kw"}__${id}__${page - 1}`;
  const tempListKey = `sdetail__${"kw"}__${id}__temp`;
    ...
    musicSdk["kw"]?.songList
      ...
      .then((result: any) => {
        ...
        if (tempList) {
          ...
          listCache.set(`sdetail__${source || "kw"}__${id}__${p}`, {
            data: {
              ...result,
              list: [
                ...tempList,
        ...
        do {
          ...
          listCache.set(`sdetail__${"kw"}__${id}__${p}`, {
            data: {
              ...result,
        ...
        return (
          listCache.get(`sdetail__${"kw"}__${id}__${page}`) as DetailPageCache
        ).data;
 ...
 * @param sortId
 */
export const setListDetailInfo: typeof songlistActions.setListDetailInfo = (
  source,
  id
) => {
  clearListDetail();
...
export const setListDetail: typeof songlistActions.setListDetail = (
  result,
  id,
  page
) => {
...
export const getListDetail = async (
  ...
  const listKey = `sdetail__${"kw"}__${id}`;
  const pageKey = `sdetail__${"kw"}__${id}__${page}`;
  ...
  return getListDetailLimit("kw", id, page);
};
