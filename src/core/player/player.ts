...
import { setItem } from "../../utils/tempStorage";
...
const { addDelayNextTimeout, clearDelayNextTimeout } =
  createDelayNextTimeout(5000);
const {
  addDelayNextTimeout: addLoadTimeout,
...
const diffCurrentMusicInfo = (
  ...
  return (
    curMusicInfo !== playerState.playMusicInfo.musicInfo || playerState.isPlay
  );
};
...
export const getMusicPlayUrl = async (
  ...
  setStatusText("player__geting_url");
  ...
  return getMusicUrl({
    ...
    onToggleSource(mInfo) {
      ...
      setStatusText("toggle_source_try");
    },
  })
    .then(async (url) => {
...
const handleRestorePlay = async (restorePlayInfo: LX.Player.SavedPlayInfo) => {
  ...
  void getPicPath({ musicInfo, listId: playMusicInfo.listId }).then(
    (url: string) => {
  ...
  void getLyricInfo({ musicInfo })
    ...
    .catch((err) => {
      console.log('error lyric', err);
      if (musicInfo.id != playMusicInfo.musicInfo?.id) return;
      setStatusText("lyric__load_error");
    });
...
const updateViewCount = async (id: number | string) => {
  await setItem("id_" + id, id, 120, async () => {
    console.log("increment ++ views", id);
    const promise = song.setViews(id).promise;
    await promise
      .then(() => {
        console.log("updated");
      })
...
const handlePlay = async () => {
  ...
  await updateViewCount(musicInfo.bdid);

  if (!playMusicInfo.isStudioPlay) {
    void getLyricInfo({ musicInfo })
      .then((lyricInfo) => {
        if (musicInfo.id != playMusicInfo.musicInfo?.id) return;
        setMusicInfo({
...
export const handleToggleStop = async () => {
  await stop();
  setTimeout(() => {
...
export const playNext = async (isAutoToggle = false): Promise<void> => {
  ...
  if (playerState.studioPlayList.length > 0) {
    const playMusicInfo = playerState.studioPlayList[0];
    setPlayMusicInfo(
      playMusicInfo.listId,
      playMusicInfo.musicInfo,
      playMusicInfo.isTempPlay,
  ...
  if (playedList.length) {
    ...
    for (
      ...
      if (
        playMusicInfo.listId == currentListId &&
        !currentList.some((m) => m.id === currentId)
      ) {
  ...
  let { filteredList, playerIndex } = filterList({
    // 过滤已播放歌曲
  ...
  global.lx.streamer.seek(0,async () => {});
  await handlePlay();
};
...
export const play = () => {
  ...
  if (isEmpty()) {
    if (playerState.playMusicInfo.musicInfo.id != global.lx.gettingUrlId)
...
const handlePlayStudio = async () => {
  ...
  //clearDelayNextTimeout()
  //clearLoadTimeout()
  //if (settingState.setting['player.togglePlayMethod'] == 'random' && !playMusicInfo.isTempPlay && !playMusicInfo.isStudioPlay) addPlayedList(playMusicInfo as LX.Player.PlayMusicInfo)
  //setMusicUrl(musicInfo)
  //await setPause();
};
