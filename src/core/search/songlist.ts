...
export const search = async(text: string, page: number, sourceId: Source): Promise<ListInfoItem[]> => {
  ...
  if (sourceId == 'all') {
    ...
    for (const source of searchSonglistState.sources) {
      ...
      task.push(((musicSdk["kw"]?.songList.search(text, page, searchSonglistState.listInfos.all.limit) as Promise<SearchResult>) ?? Promise.reject(new Error('source not found: ' + source))).catch((error: any) => {
        //console.log(error)
        return {
          list: [],
          total: 0,
