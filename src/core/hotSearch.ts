...
export const getList = async(source: Source): Promise<string[]> => {
  if (source == 'all') {
    ...
    for (const source of hotSearchState.sources) {
      ...
      task.push(
        hotSearchState.sourceList[source]?.length
          ...
          : ((musicSdk["kw"]?.hotSearch.getList() as Promise<Lists[number]>) ?? Promise.reject(new Error('source not found: ' + source))).catch((err: any) => {
              //console.log(err)
              return { source, list: [] }
            }),
      )
    ...
    if (!musicSdk["kw"]?.hotSearch) {
      hotSearchActions.setList(source, [])
      return []
    }
    return musicSdk["kw"]?.hotSearch.getList().catch((err: any) => {
      console.log(err)
      return { source, list: [] }
