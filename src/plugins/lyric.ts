...
import Lyric from '../plugins/lrc-parser'
// import { getStore, subscribe } from '../../../../store'
export type Line = any[number]
type PlayHook = (line: number, text: string) => void
type SetLyricHook = (lines: any) => void
...
export const useLrcSet = () => {
  const [lines, setLines] = useState<any>(lrcTools.currentLines)
  useEffect(() => {
    const callback = (lines: any) => {
      setLines(lines)
    }
